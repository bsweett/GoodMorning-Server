...
public class HibernateTaskManager extends HibernateDatabaseManager {
	...
	private final String CREATE_TASK_TABLE = "create table " + getTableName() + "(TASK_ID_PK char(36) primary key, NAME tinytext,"
			+ "CREATION_TIME timestamp, NEXT_ALERT_TIME timestamp, ALERT_TIME time, TASK_TYPE enum('CHORE', 'TRAVEL', 'ENTERTAINMENT', 'ALARM', 'UNKNOWN'),"
			+ "DEEP_LINK enum('FACEBOOK', 'YOUTUBE', 'MAPS', 'SMS', 'MUSIC', 'BOOKS', 'NONE'), SOUND tinytext," 
	...
	private final String SELECT_TASK_WITH_TASKNAME_AND_TYPE = "from " + getClassName() + " as task where task.name = :name and task.taskType = :taskType";
	...
	@SuppressWarnings("unchecked")
	public synchronized Task getTaskByNameAndType(String name, TaskType taskType) {
		...
		try {
			session = HibernateUtility.getCurrentSession();
			transaction = session.beginTransaction();
			Query query = session.createQuery(SELECT_TASK_WITH_TASKNAME_AND_TYPE);
