...
public class HibernateRSSFeedManager extends HibernateDatabaseManager {
	...
	private final String SELECT_FEED_WITH_SOURCE = "from " + getClassName() + " as rssfeed where rssfeed.source = :source";
	...
	@SuppressWarnings("unchecked")
	public synchronized RSSFeed getFeedBySource(String url) {
		...
		try {
			session = HibernateUtility.getCurrentSession();
			transaction = session.beginTransaction();
			Query query = session.createQuery(SELECT_FEED_WITH_SOURCE);
			query.setParameter("source", url);
			List<RSSFeed> feeds = query.list();
			transaction.commit();
